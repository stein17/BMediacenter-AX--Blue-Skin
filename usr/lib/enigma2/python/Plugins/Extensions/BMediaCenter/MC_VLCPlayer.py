# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/BMediaCenter/MC_VLCPlayer.py
...
class MC_VLCServerlist(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions',
         'MenuActions',
         'ShortcutActions',
         'MoviePlayerActions'], {'back': self.Exit,
         'red': self.keyDelete,
...
class MC_VLCMedialist(Screen):
    ...
    def __init__(self, session, server):
        ...
        self.curfavfolder = -1
        self.__event_tracker = ServiceEventTracker(screen=self, eventmap={iPlayableService.evEnd: self.StopPlayback,
        ...
        self['actions'] = ActionMap(['WizardActions',
         ...
         'leavePlayer': self.StopPlayback}, -1)
    ...
    def update(self):
        try:
            ...
            self.session.open(MessageBox, _('Error updating file- and playlist from server %s:\n%s' % (self.server.getName(), e)), MessageBox.TYPE_ERROR)
    ...
    def keyFilter(self):
        ...
        try:
            ...
            self.session.open(MessageBox, _('Error updating filelist from server %s:\n%s' % (self.server.getName(), e)), MessageBox.TYPE_ERROR)
        ...
        if media is not None:
            ...
            if media.lower().endswith('.m3u') or media.lower().endswith('.pls') or media.lower().endswith('.xspf'):
                try:
                    ...
                    self.session.open(MessageBox, _('Error loading playlist %s into server %s:\n%s' % (media, self.server.getName(), e)), MessageBox.TYPE_ERROR)
    ...
    def getFilesAndDirsCB(self, currentDirectory, regex):
        try:
            ...
            self.session.open(MessageBox, _('Error loading playlist into server %s:\n%s' % (self.server.getName(), e)), MessageBox.TYPE_ERROR)
            raise ExpatError, e
        except Exception as e:
            self.session.open(MessageBox, _('Error loading filelist into server %s:\n%s' % (self.server.getName(), e)), MessageBox.TYPE_ERROR)
...
class MC_VLCFavoriteFolders(Screen):
    ...
    def __init__(self, session):
        ...
        for i in range(0, config.plugins.mc_vlc.foldercount.value):
            ...
            self.list.append((cfg.name.value,
             int(i),
             cfg.basedir.value,
             '50'))
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions'], {'ok': self.JumpToFolder,
         'cancel': self.Exit,
         'red': self.keyDelete,
         'green': self.FavoriteFolderAdd,
         'yellow': self.FavoriteFolderEdit,
         'blue': self.ResetAll}, -1)
