# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/BMediaCenter/MC_AudioPlayer.py
...
class MC_AudioPlayer(Screen, HelpableScreen, InfoBarSeek):
    ...
    def __init__(self, session):
        ...
        self['actions'] = HelpableActionMap(self, 'MC_AudioPlayerActions', {'ok': (self.KeyOK, 'Play selected file'),
         ...
         'previous': (self.KeyPrevious, 'Previous song')}, -2)
    ...
    def addDirtoPls(self, directory, recursive = True):
        ...
        for x in filelist.getFileList():
            if x[0][1] == True:
                return
            if filelist.getServiceRef() and filelist.getServiceRef().type == 4097:
...
class MC_AudioPlaylist(Screen, InfoBarSeek):
    ...
    def __init__(self, session):
        ...
        self.addPlaylistParser(PlaylistIOPLS, 'pls')
        self.addPlaylistParser(PlaylistIOInternal, 'e2pls')
        self.__event_tracker = ServiceEventTracker(screen=self, eventmap={iPlayableService.evEOF: self.fileupdate,
         iPlayableService.evUser + 14: self['screensaver'].screensaver})
...
class Lyrics(Screen):
    ...
    def __init__(self, session):
        ...
        self.__event_tracker = ServiceEventTracker(screen=self, eventmap={iPlayableService.evUser + 11: self['coverly'].coverlyrics})
        self['actions'] = HelpableActionMap(self, 'MC_AudioPlayerActions', {'cancel': self.Exit,
         'up': self.pageUp,
         'left': self.pageUp,
         'down': self.pageDown,
...
class MediaPixmap(Pixmap):
    ...
    def __init__(self):
        ...
        self.coverFileNames = ['cover.jpg', 'folder.png', 'folder.jpg','Cover.jpg', 'Folder.png', 'Folder.jpg']
