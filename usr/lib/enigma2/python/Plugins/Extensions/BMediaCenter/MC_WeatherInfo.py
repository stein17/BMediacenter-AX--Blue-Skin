# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/BMediaCenter/MC_WeatherInfo.py
...
class MC_WeatherInfo(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'DirectionActions', 'ColorActions'], {'back': self.exit,
         'blue': self.config,
         'right': self.nextItem,
         'left': self.previousItem}, -1)
        ...
        for childs in root:
            ...
            for items in childs:
                if items.tag == 'current':
                    self['currentTemp'].text = '%s\xc2\xb0%s' % (items.attrib.get('temperature').encode('utf-8', 'ignore'), degreetype)
                    ...
                    self['feelsliketemp'].text = _('Feels like %s') % items.attrib.get('feelslike').encode('utf-8', 'ignore') + '\xc2\xb0' + degreetype
                    ...
                    self['weekday%s_temp' % index].text = 'Min: %s\xc2\xb0%s \n Max: %s\xc2\xb0%s' % (lowTemp,
                     degreetype,
                     highTemp,
                     degreetype)
