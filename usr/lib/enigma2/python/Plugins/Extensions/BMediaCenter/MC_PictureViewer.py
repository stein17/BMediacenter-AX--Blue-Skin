# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/BMediaCenter/MC_PictureViewer.py
...
class MC_PicSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['SetupActions', 'DirectionActions'], {'cancel': self.exit,
         'save': self.exit,
         'ok': self.exit,
         'left': self.keyLeft,
    ...
    def createsetup(self):
        list = [getConfigListEntry(_('Slideshow Interval (sec.)'), config.plugins.mc_pp.slidetime),
        ...
        if config.plugins.mc_pp.musicenable.value == True:
            sublist = [getConfigListEntry(_('Select music'), config.plugins.mc_pp.music)]
            list.extend(sublist)
